...
class MyServerProtocol(WebSocketServerProtocol):
	...
	def __init__(self, *args, **kwards):
		...
		self.random_intros = ["Fuck! ", "Fuck, fuck, fuck! ", "Oh, fuck! "]
	...
	def onConnect(self, request):
		self.telegram_interface.send_update("New node created")
	...
	def onMessage(self, payload, isBinary):
		if kill_signal_triggered:
			...
			node_msg = payload.decode('utf8')
			self.telegram_interface.send_update(random.choice(self.random_intros) + node_msg)
			#print("message from market analyser is: ", grabber_client_msg)
			return
